								...
								u32* reg_table, u32 reg_table_size)
{
	...
	cam_info("%s reg start=0x%pK, reg end=0x%pK", __func__,
				(void*)reg_table[0], (void*)reg_table[reg_table_size-1]);

	memset(name, 0, sizeof(name));
#ifdef MINI_ISP_DEBUG_DUMP_LOG
	do_gettimeofday(&timeval);
	snprintf(name, sizeof(name),
		"/data/camera/misp_0x%pK_%08d.regx",(void*)reg_table[0],(int)timeval.tv_sec);
#else
	snprintf(name, sizeof(name), "/data/camera/misp_0x%pK.regx",(void*)reg_table[0]);
#endif
	...
	for (index = 0; index < reg_table_size; index++) {
		...
		*pcount = 10;
		...
		snprintf(reg_head, sizeof(reg_head)-1, "[0x%pK]", (void*)reg_table[index]);
								...
								u32 start, u32 total, u32 mode)
{
	...
	cam_info("%s start=0x%pK, total=%u, mode=%u", __func__, (void*)start, total, mode);

	memset(name, 0, sizeof(name));
	snprintf(name, sizeof(name), "/data/camera/misp_0x%pK.ram", (void*)start);
										...
										u32 addr, u32 val)
{
	...
	cam_info("%s addr=0x%pK, val=0x%x", __func__, (void*)addr, val);
