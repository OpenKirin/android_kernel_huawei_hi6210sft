...
/* DTS2014083001453 add by l00269429 for all focus function 201400901  begin */
#define   VCM_DEFAULT_CODE  0x1a0
#define   VCM_MAX_CODE          0x2b0
#define   VCM_MIN_CODE          0xa0
/* DTS2014083001453 add by l00269429 for all focus function 201400901  end */

/* with stabilizer, max size is larger than 1080P. */
/* added by y00215412 for DTS2012092500472 */
...
/* added for DTS2013022701160 by j00212990 in 2013-02-27 */
u32 raw_unit_area;
...
/* added for DTS2012051506506 by j00212990 2012-05-31 */
static void ispv1_assistant_af(bool action);
/* Changed for DTS2012121005733 by y00215412 2012-12-10 */
...
/* added for DTS2013041904698 by j00212990 2013-04-26 */
...
/*< DTS2013082004832 modified by z00195335(w00223743) 2013-08-20 begin */
static af_run_stage ispv1_af_search_top(af_run_param *af_info, pos_info *curr, vcm_info_s *vcm, bool coarse, int first_contrast);
/* DTS2013082004832 modified by z00195335(w00223743) 2013-08-20 end >*/
...
/* Added by y00215412 2012-11-27 for DTS2012112401647 */
...
/*< DTS2013082004832 added by z00195335(w00223743) 2013-08-20 begin */
...
/* DTS2013082004832 added by z00195335(w00223743) 2013-08-20 end >*/
...
/* Changed by y00215412 2012-10-27 for DTS2012102604902 */
...
/* added for DTS2013041904698 by j00212990 2013-04-24 */
...
/* added for DTS2013021801158 by j00212990 in 2013-02-21 */
...
/* added for DTS2013041904698 by j00212990 in 2013-04-26 */
...
/* Added for DTS2012091002966 by y00215412 2012-09-10 */
static void ispv1_cal_vcm_range(vcm_info_s *vcm)
{
	...
	if (vcm->get_vcm_otp != NULL) {
		...
		if (otp_end != 0) {
			...
			/*< DTS2013092901239 added by z00195335(w00223743) 2013-09-29 begin */
			if ( (otp_start & 0x3f0) < (2 * vcm->normalStep) ) {
				vcm->infiniteDistance = (2 * vcm->normalStep);
			} else {
			/* DTS2013092901239 added by z00195335(w00223743) 2013-09-29 end >*/
 ...
 * Output      : NA;
 * ReturnValue : NA;
 * Other       : added for DTS2013011405432 by j00212990 in 2013-01-14;
 **************************************************************************
 */
static int ispv1_setreg_vcm_code_done(vcm_info_s *vcm, u32 vcm_code)
{
...
#else
	/*< DTS2013110604002 modified by w00223743 2013-11-06 begin */
	/*
	...
	/* DTS2013110604002 modified by w00223743 2013-11-06 end> */
#endif
	return ret;
}
 ...
 * Other       : last modified for DTS2013011604263 by j00212990 in 2013-01-16;
 **************************************************************************
 */
static bool ispv1_setreg_vcm_code(u32 dest_code)
{
	...
	if ((dest_code < safe_position) && (curr_code > safe_position)
		...
		if (vcm->vcm_type == VCM_AD5823) {
			...
			max_step_cnt = 2;
		/*< DTS2013092901239 modified by z00195335(w00223743) 2013-09-29 begin */
		} else if (vcm->vcm_type == VCM_DW9714_Sunny || vcm->vcm_type == VCM_DW9714_Liteon){
		/* DTS2013092901239 modified by z00195335(w00223743) 2013-09-29 end >*/
...
static void ispv1_focus_vcm_go_infinite(u8 delay_flag)
{
	...
	/* last modified for DTS2013011405432 by j00212990 in 2013-01-14 */
...
int ispv1_focus_init(void)
{
	...
	/* revised by y00215412 for DTS2012092500472 */
	/*< DTS2013110806156 modified by w00223743 2013-11-12 begin */
	size = MAX_PREVIEW_WIDTH * MAX_PREVIEW_HEIGHT *
		this_focus_area->percent_w * this_focus_area->percent_h / 100 / 100 * 9;
	/* DTS2013110806156 modified by w00223743 2013-11-12 end >*/
	/* Modified for DTS2012092104131 by y00215412 */
	...
	sema_init(&(afae_ctrl->af_run_sem), 1); /* added for DTS2013041904698 by j00212990 2013-04-26 */
...
{
	/* Modified for DTS2012050905799, delete for speed up by y00215412 */
	/* ispv1_wait_caf_done(); */
 ...
 * Other       : last modified for DTS2013041904698 by j00212990 2013-04-26;
 **************************************************************************
 */
...
static void ispv1_af_start_work_func(struct work_struct *work)
{
	...
	if ((cur_focus_mode == CAMERA_FOCUS_CONTINUOUS_PICTURE)
		...
		/* Added for DTS2013041904698 by j00212990 in 2013-04-08 */
...
static int ispv1_get_yuvrect_of_app(pic_attr_t *pic_attr, camera_rect_s *rect, u32 zoom)
{
	...
	coordinate_s phy_center;
	int left, top;
    /*DTS2013100903710 modify by s61250 20131010 begin*/
    effect_params *effect = get_effect_ptr();
    focus_area_param *this_focus_area = &effect->af_param.focus_area;
    u32 preview_width = pic_attr->out_width;
    u32 preview_height = pic_attr->out_height;
    /*DTS2013100903710 modify by s61250 20131010 end*/
...
static int ispv1_focus_adjust_yuvrect(camera_rect_s *yuv, u32 preview_width, u32 preview_height, u32 zoom)
{
	...
	/* added for DTS2012112101774 by y00215412 20121121: set limitation for MAX focus rect start. */
	...
	yuv->width = width;
	yuv->height = height;
	/* added for DTS2012112101774 by y00215412 20121121: set limitation for MAX focus rect end. */
	...
	if (yuv->height < (preview_height / height_ratio)) {
		...
		/* added for DTS2012120405574 by y00215412 20121206: make sure adjusted rect is true. */
...
/* Last modified for DTS2012060403229 by j00212990 */
...
int ispv1_set_vcm_parameters(camera_focus focus_mode)
{
	...
	case CAMERA_FOCUS_CAF_FORCE_AF: /* Added for DTS2013041904698 by j00212990 in 2013-04-08 */
	...
	case CAMERA_FOCUS_AUTO_VIDEO: /* added for touch af in video mode by j00212990 2013-01-24 */
		...
		vcm->moveRange = RANGE_NORMAL;
		break;
	/* Added for DTS2012052902948 by j00212990 2012-06-06 */
 ...
 * Description : modified for for DTS2012051001096;
 * Input       : NA;
 * Output      : NA;
 * ReturnValue : NA;
 * Other       : NA;
...
int ispv1_set_focus_area(focus_area_s *area, u32 zoom)
{
	...
	if (get_focus_state() == FOCUS_STATE_CAF_RUNNING ||
		...
		if (zoom == afae_ctrl->zoom) {
			/* revised by y00215412 2012-11-23 for DTS2012112300233 */
...
static int ispv1_set_focus_area_done(focus_area_s *area, u32 zoom)
{
	...
	/* Added for DTS2012060403229 by j00212990 */
...
setreg_out:
	...
	/* modified for DTS2012060403229 by j00212990 */
...
int ispv1_set_metering_area(metering_area_s *area, u32 zoom)
{
	...
	if (roi_flag == true) {
		...
		if (enhance_flag == true) {
			...
			/*modified for DTS2013110404011 by q00215342 2013-11-04 begin*/
			//restore_target_high();
			/*modified for DTS2013110404011 by q00215342 2013-11-04 end*/
		} else {
 ...
 * Output      : NA;
 * ReturnValue : NA;
 * Other       : Added for DTS2012051001096 ;
 **************************************************************************
 */
 ...
 * Output      : NA;
 * ReturnValue : NA;
 * Other       : added for DTS2012051506506 by j00212990 2012-05-31;
 *               modified for DTS2012062501039 by j00212990 2012-06-25, decrease the level of luminance
 **************************************************************************
 */
...
static void ispv1_assistant_af(bool action)
{
	...
	if (true == action) {
		/* modified for DTS2012062501039 by j00212990 2012-06-25 */
		/* Added for DTS2012121005733 by y00215412 2012-12-10 */
 ...
 * Output      : NA;
 * ReturnValue : NA;
 * Other       : added for DTS2013041904698 by j00212990 2013-04-26;
 **************************************************************************
 */
static bool ispv1_af_flash_needed(camera_sensor *sensor, camera_flash flash_mode, lum_win_info_s lum_info)
{
...
static void ispv1_af_flash_check_open(void)
{
	...
	/* DTS2014092300149 add by l00269429 for switch flash mode issue 201400923  begin */
	else{
             this_ispdata->assistant_af_flash = false;
	}
	/* DTS2014092300149 add by l00269429 for switch flash mode issue 201400923  end */
}
 ...
 * Other       : added for DTS2012120301586 by j00212990 in 2012-12-27;
 **************************************************************************
 */
	...
	if (force_start && unpeace == 0) {
		if (mean_data->lum >= this_caf_trigger->min_trigger_lum) {
			if ((afae_ctrl->focus_stat_frames > this_caf_trigger->stat_skip_frame)
				...
				/*
				  * added by j00212990 for DTS2012120301586 in 2012-12-27
...
static u32 ispv1_focus_calc_edge(u8 *pdata, camera_rect_s *rect, int contrast_shift, bool coarse_flag)
{
	...
	/*< DTS2013101901265 added by w00223743 2013-10-29 begin */
	...
	int width_shift;
	int step_width;
	/* DTS2013101901265 added by w00223743 2013-10-29 end >*/
...
void ispv1_wakeup_focus_schedule(bool force_flag)
{
	...
	/* check if focus run function need wake up. revised by y00215412 for DTS2012092004946 */
...
void ispv1_k3focus_run(void)
{
	...
	/* DTS2014083001955 modify by l00269429 2014-08-30 begin */
	pos_info large_contrast_array[50];
	/* DTS2014083001955 modify by l00269429 2014-08-30 end */
	u32 large_curr = 0;
	pos_info large_top;
	...
	focus_position_e pos_part;
       /* DTS2014083001955 modify by l00269429 2014-08-29 begin */
	u32 contrast_array[50];
	/* DTS2014083001955 modify by l00269429 2014-08-29 end */
			...
			case VCAF_RUN_STAGE_REWIND:
				if (curr.code != top->code) {
					...
					if (get_focus_state() == FOCUS_STATE_CAF_RUNNING)
						set_focus_state(FOCUS_STATE_CAF_DETECTING);
					set_focus_result(STATUS_FOCUSED);
					/* Added by y00215412 2012-11-27 for DTS2012112401647 begin */
					ispv1_focus_get_curr_data(&curr_data);
					restart = ispv1_check_caf_need_restart(&start_data, &curr_data);
					if (restart == true)
						set_caf_forcestart(CAF_FORCESTART_FORCEWAIT);
					/* Added by y00215412 2012-11-27 for DTS2012112401647 end */
...
static void ispv1_vcaf_range_cal(af_run_param *af_info, pos_info *curr, af_run_stage next_stage, FOCUS_STATUS result)
{
	...
	case VCAF_RUN_STAGE_REWIND:
		/* modified by j00212990 for DTS2012120301586 in 2012-12-28 */
...
/* DTS2013082004832 modified by z00195335(w00223743) 2013-08-20 end >*/
{
	...
	if (((curr->contrast <= get_af_judge_threshold_low(top->contrast))
	   ...
	   || (curr->contrast <= top->contrast && check_vcmcode_is_edge(trip, curr->code) == true)) {
		...
		curr->code = next_code;
		trip->step_cnt++;
		/*< DTS2013082004832 deleted by z00195335(w00223743) 2013-08-20 begin */
		//ispv1_setreg_vcm_code(curr->code);
		/* DTS2013082004832 deleted by z00195335(w00223743) 2013-08-20 end >*/
...
bool enlarge_focus_window(camera_rect_s *rect, int width, int height, int zoom)
{
	...
	/*< DTS2013091606958 modified by w00223743(z00195335) 2013-09-16 begin */
	/*
	...
	/* DTS2013091606958 modified by w00223743(z00195335) 2013-09-16 end >*/
