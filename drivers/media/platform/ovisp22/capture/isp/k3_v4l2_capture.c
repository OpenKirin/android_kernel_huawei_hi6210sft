...
/* BEGIN PN:DTS2014061701601, Added by l00183577, 2014-06-17 */
static struct dentry *debugfs_sensor_reg;//sensor reg read/write interface
/* END PN:DTS2014061701601, Added by l00183577, 2014-06-17 */
#endif
#endif
				      ...
				      struct v4l2_buffer *b, struct ion_client *ion_client, int share_fd)
{
	...
	if (IS_ERR(a->ion_handle) || a->ion_handle == NULL) {
		print_error("%s, ion_import_dma_buf fail.ion_handle=%pK.ion_client=%pK,a->share_fd=%d.\n",
					__func__, (void*)a->ion_handle, (void*)ion_client, a->share_fd);
		return;
	}
	...
	print_debug("%s: frame phyaddr=0x%pK, viraddr=%pK", __func__, (void*)a->phyaddr, a->viraddr);
}
				...
				struct v4l2_control *a)
{
	...
	case V4L2_CID_GET_METERING_FLASH_RESULT:
		...
		/*DTS2012042105985 added y00215412 2012-05-14 */
				...
				struct v4l2_control *v4l2_param)
{
	...
	case V4L2_CID_FOCUS_AUTO:
		{
			/* modified for DTS2013041904698 by j00212990 2013-04-24 */
			if (v4l2_param->value)
				     ...
				     struct v4l2_ext_controls *arg)
{
	...
	/* revise by c00220250 for DTS2014011706576, 2014-01-18 begin*/
	void * data_buf                     = NULL;
	/* revise by c00220250 for DTS2014011706576, 2014-01-18 end*/
		...
		case V4L2_CID_BUFFER_OFFSET:
			{
				if (controls[cid_idx].value == 0) {
					...
					goto end;
                                ...
                                /* Modified  by w00199382 for DTS2013042503069, 2013/5/3, begin */
                if ( 0 != controls[cid_idx].value)
				        ...
				        reg_buf = true;
                                ...
                                /* Modified  by w00199382 for DTS2013042503069, 2013/5/3, end */
				  ...
				  enum v4l2_buf_type buf_type)
{
    ...
    if (ret == 0)
    {
        ...
        /* modified by c00144034 for DTS2013102103522 begin */
        ...
        /* modified by c00144034 for DTS2013102103522 end */
...
static int k3_init_v4l2_ctl(v4l2_ctl_struct *v4l2_ctl)
{
	...
	print_debug("enter %s, cam 0x%pK", __func__, (void*) v4l2_ctl);
...
static int k3_v4l2_resume(struct platform_device *pdev)
{
	...
	if (0 == v4l2_ctl.open_count) {
        ...
        /* add ddr lock in camera resume interface, DTS2013090509598, modifier: z62576, begin*/
        //k3_isp_lock_ddr_freq()/* req ddr freq lock */;
        /* add ddr lock in camera resume interface, DTS2013090509598, modifier: z62576, end*/

		/* init all queues */
		/* init camera data struct here on first open */

		/*del by c00220250 for DTS2014101002670, 2014-10-10, begin*/
		...
		//memset(&isp_rsv_frame[0], 0x00, sizeof(isp_rsv_frame));
		/*del by c00220250 for DTS2014101002670, 2014-10-10, end*/
...
static int k3_v4l2_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (0 != v4l2_ctl.open_count) {
        ...
        /* add ddr unlock in camera suspend interface, DTS2013090509598, modifier: z62576, begin*/
        k3_isp_unlock_ddr_freq()/* cancel and release ddr freq lock */;
        /* add ddr unlock in camera suspend interface, DTS2013090509598, modifier: z62576, end*/
	}
