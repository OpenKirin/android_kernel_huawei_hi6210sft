...
/*
**************************************************************************
* FunctionName: k3_socgpio_get_id_value_float;
* Description : get sensor id value;
* Input         :gpio_name:camera senor id_pin's name;
* Output       :value:camera sensor id_pin's value,ether 0 or 1,if ret < 0 ,it can not get id_pin value   ;
* ReturnValue:ret:if ret < 0 means error ,can not get right id_pin's value,if not ,means the value is right;
...
**************************************************************************
*/
camera_pin_state k3_socgpio_get_id_value_float(char *gpio_name)
{
	int ret = -EINVAL;
	struct device_node *np = NULL;
	struct platform_device *pdev =NULL;
	...
	//pull down
	ret = ispio_hw_ctl->idconfig(PULL_DN);
	if (ret < 0) {
		print_error("%s: idconfig pull down is fail",__func__);
		goto ERROR;
	}
	udelay(10);
	...
	//pull up
	ret = ispio_hw_ctl->idconfig(PULL_UP);
	if (ret < 0) {
		print_error("%s: idconfig pull down is fail",__func__);
		goto ERROR;
	}
	...
	value_pu = ret;
	print_info("%s  pull dn:%d, pull up:%d", __func__, value_pd, value_pu);

	if (value_pd != value_pu){
		gpio_free(gpio_pin);
		return PIN_FLOAT;
	}
	else{
...
EXPORT_SYMBOL(k3_socgpio_get_id_value_float);
...
int k3_ispio_init(struct platform_device *pdev, struct platform_device *module_pdev)
{
	...
	ret = ispio_hw_ctl->ispio_hw_init(pdev, module_pdev);
	if (ret == 0)
