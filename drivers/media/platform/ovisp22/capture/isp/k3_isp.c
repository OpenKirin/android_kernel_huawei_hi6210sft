...
/*< DTS2013080206195 z00195335 20130807 add for superzoom begin */
#include "hwa_cam_tune_common.h"
/* DTS2013080206195 z00195335 20130807 add for superzoom end >*/
...
/* read ddr max freq from file, DTS2013122702236, z62576, 20131227, begin */
...
/* read ddr max freq from file, DTS2013122702236, z62576, 20131227, end */

/* VARIABLES AND ARRARYS */
/* BEGIN PN: DTS2015110304954,Modified by jwx206032, 2015/11/03 */
k3_isp_data isp_data;
/* END   PN: DTS2015110304954,Modified by jwx206032,  2015/11/03 */
...
#else
	/* Modified for DTS2012070300448 by j00212990 2012-07-06 using for recognise shooting mode */
	if ((isp_data.sensor->sensor_index == CAMERA_SENSOR_PRIMARY) && (flashlight != NULL) && (CAMERA_SHOOT_SINGLE == isp_data.shoot_mode)) {
		...
		if (FLASH_ON == state) {
			if (((isp_data.flash_mode == CAMERA_FLASH_AUTO) && (true == isp_hw_ctl->isp_is_need_flash(isp_data.sensor))) ||
				...
				/*
				 * deleted by y00215412 DTS2012062808862 2012-07-03
...
/* added by c00144034 for DTS2014050406464 begin */
/*
 **************************************************************************
...
/* added by c00144034 for DTS2014050406464 end */
		     ...
		     enum v4l2_buf_type buf_type, camera_state state,buffer_arr_t *buf_arr)
{
	...
	/* BEGIN PN:DTS2014060700725, Added by l00183577, 2014-06-07 */
	if (CAMERA_ZSL_OFF == k3_isp_get_zsl_state()
	...
	/* END PN:DTS2014060700725, Added by l00183577, 2014-06-07 */
	...
	//print_ddr();	/* add by j00179721 20120621, deleted by y00215412 DTS2012121500748 20121215 for MCU panic */
	return ret;
}
...
{
	/* BEGIN PN:DTS2014060700725, Modified by l00183577, 2014-06-07 */
    ...
    if (CAMERA_ZSL_ON == k3_isp_get_zsl_state()){
        ...
        if(hwa_hwscope.mode == HW_SCOPE_ON){
            ...
            /* DTS2013092407289 z00195335 20130924 modified for superzoom quick capture in ZSL end >*/
    ...
    /*< DTS2013080206195 z00195335 20130807 add for superzoom begin */
    if (CAMERA_ZSL_ON == k3_isp_get_zsl_state()){
        /*< DTS2013092407289 z00195335 20130924 modified for superzoom quick capture in ZSL begin */
        if(hwa_hwscope.mode == HW_SCOPE_ON){
    ...
    /* DTS2013080206195 z00195335 20130807 add for superzoom end >*/
    return ret;
	/* END PN:DTS2014060700725, Modified by l00183577, 2014-06-07 */
...
void k3_isp_auto_focus(int flag)
{
	...
	/* END   PN: DTS2015110304954,Modified by jwx206032,  2015/11/03 */
	/* modified for DTS2012051506506 by j00212990 2012-05-31
...
int k3_isp_set_saturation(camera_saturation saturation)
{
	...
	/* Modified for DTS2012091403785 by w00225845 2012-09-14 using for support s5k5cag sensor */
	if (NULL != camera_tune_ops)
	...
	case CAMERA_FLASH_TORCH:
		{
			...
			/* DTS2014082503363 add by l00269429 for switch flash mode issue 201400901  begin */
			isp_hw_ctl->isp_set_auto_flash(0, flash_mode);
			/* DTS2014082503363 add by l00269429 for switch flash mode issue 201400901  end */
			break;
		}
	case CAMERA_FLASH_OFF:
		{
			print_info("set camera flash OFF MODE");
			/* DTS2014092300125 add by l00269429 for switch flash mode issue 201400923  begin */
			if (((isp_data.flash_mode == CAMERA_FLASH_TORCH) || (isp_data.flash_mode == CAMERA_FLASH_AUTO) || (isp_data.flash_mode == CAMERA_FLASH_ON)
				) && flashlight) {
				flashlight->turn_off();
			}
			/* DTS2014092300125 add by l00269429 for switch flash mode issue 201400923  end */
...
static void k3_isp_calc_zoom(camera_state state, scale_strategy_t scale_strategy, u32 *in_width, u32 *in_height)
{
	...
	if (isp_data.zoom != 0) {
		/* Modified  by w00199382 for DTS2013091205182, 2013/9/17, begin */
		if ((YUV_SCALE_DIVIDEND !=  attr->yuv_up_scale_nscale)
		||(YUV_SCALE_DIVIDEND !=  attr->yuv_down_scale_nscale)
		||(1 != attr->yuv_dcw)){

			/* Modified  by w00199382 for DTS2013071900421, 2013/7/22, begin */
			...
			/* Modified  by w00199382 for DTS2013071900421, 2013/7/22, end */
		}else{
 ...
 * Output      : NA;
 * ReturnValue : NA;
 * Other       : Added for DTS2012070300448 by j00212990 using for recognise shot mode  ;
 **************************************************************************
 */
...
static void k3_isp_set_default(void)
{
	...
	/* added for DTS2012052902948 by j00212990 */
	isp_data.assistant_af_flash = false;
	isp_data.af_need_flash = false; /* added for DTS2013041904698 by j00212990 2013-04-26 */

	/* Added for DTS2012070300448 by j00212990 2012-07-06 using for recognise shot mode	*/
...
/* add ddr lock/unlock in camera suspend/resume interface, DTS2013090509598, modifier: z62576, begin*/
...
/* add ddr lock/unlock in camera suspend/resume interface, DTS2013090509598, modifier: z62576, end*/
