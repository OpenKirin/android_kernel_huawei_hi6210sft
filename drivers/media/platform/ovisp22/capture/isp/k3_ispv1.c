...
const struct isp_reg_t isp_mipi_init_regs1[] = {
	...
	/* y00215412 added for DTS2012083103554 2012-10-26 */
...
/* DTS2012050501812 modify by w00185358 BEGIN */
...
/* DTS2012050501812 modify by w00185358 END */
/* Modified  by w00199382 for DTS2013080801738, 2013/8/31, begin */
...
/* Modified  by w00199382 for DTS2013080801738, 2013/8/31, end */
...
static u16 ispv1_convert_output_fmt(u32 cmdset, u32 fmt)
{
	...
	case V4L2_PIX_FMT_YUYV:
		...
		/* Modified for DTS2013082100046 by c00220250 */
...
/* Modified for DTS2012091403785 by w00225845 2012-09-14 using for support s5k5cag sensor begin */
...
/* Modified for DTS2012091403785 by w00225845 2012-09-14 using for support s5k5cag sensor end */
...
static void ispv1_disable_irq(void)
{
    /* Enable INT before send cmd and disable INT at stop cmd, DTS2013082006383, modifier: z62576, begin */
    ...
    /* Enable INT before send cmd and disable INT at stop cmd, DTS2013082006383, modifier: z62576, end */

	/* DTS2012021801140 changed by y36721 2012-02-22 */
...
/* DTS2014070102234:use reserved buf to avoid drop when no ready buf available.begin */
/*
 **************************************************************************
...
/* DTS2014070102234:use reserved buf to avoid drop when no ready buf available.end */
...
static int ispv1_set_zoom(zoom_t *zoom, zoom_quality_t quality)
{
	...
	if (ret != 0){
	    ...
	    /* modified by c00144034 for DTS2014072402655 */
	    ret = 0;
	}
	/* Modified  by w00199382 for DTS2013082008650, 2013/9/04, end */
...
void ispv1_refresh_yuv_crop_pos(void)
{
	...
	/* BEGIN: Modified for PN:DTS2013082909267 by w65960, 2013/9/12 */
	/* BEGIN: Modified for PN:DTS2013101605902 by c00144034, 2013/10/22 */
    x = (x) & (~3);
    y = (y) & (~3);
 ...
 * Description : added for DTS2012061807407 by j00212990 using for denoise in preview
...
#if 0
/* y00215412 DTS2012081007721 2012-08-10 add begin */
...
/* y00215412 DTS2012081007721 2012-08-10 add end */
#endif

/* add by y00215412 for DTS2012112605318 2012-11-30 begin */
...
/* add by y00215412 for DTS2012112605318 2012-11-30 end */
	...
	camera_sensor *sensor, bool flash_on, camera_scene scene)
{
	...
	/* added by y00215412 for DTS2012070902844 2012-07-11 */
	u32 max_expo;
	 ...
	 */
	/* Revised by y00215412 for DTS2012090702901 */
	frame_index = sensor->capture_frmsize_index;

	/* added by y00231328 for DTS2012102604572 to set CCM gain when awb and ae changed  2012-11-1 */
	...
	*/
	...
	/* Modified for DTS2012072400856 by j00212990 in 2012-07-24. */
	...
	if (true == flash_on) {
		...
		/* modified for DTS2012060806041 by j00212990 2012-06-08 */
	...
	if (CAMERA_SCENE_ACTION == scene) {
		...
		/* revised by y00215412 for DTS2012112605318 2012-11-30 */
		if ((CAMERA_ISO_AUTO == this_ispdata->iso) && (1 == count)) {
			if (false == flash_on) {
				...
				/*
				 * added for DTS2012072400856 by j00212990 in 2012-07-24.
	...
	/* Modified for DTS2012091403785 by w00225845 2012-09-14 using for support s5k5cag sensor end */
	/* Add for DTS2014070201302 by c00220250 2014-08-15, begin */
	...
	/* Add for DTS2014070201302 by c00220250 2014-08-15, end */
	...
	if ((isp_data.pro_mode) && (isp_data.pro_cap_expo)){
		...
		|| (false == sensor->support_summary)){
		    /* Modified for DTS2013103004501 by c00220250, 2013/10/30,begin */
			max_gain = sensor->max_gain * min_cap_fps / min_fps;
			CMD_SET_ISP_SET_GAIN_RANGE(max_gain, sensor->min_gain);
		    /* Modified for DTS2013103004501 by c00220250, 2013/10/30,end */
		}
		else {
	...
	/* DTS2012021900381 y36721 2012-03-05 begin */
	switch (count) {
	case 1:
	...
	/* DTS2012021900381 y36721 2012-03-05 end */
	...
	/* modified by j00179721 for DTS2012041403018 20120620 begin*/
	if (true == flash_on) {
	...
	/* modified by j00179721 for DTS2012041403018 20120620 end*/
	...
	/* Modified for DTS2014060908426 by c00144034, 2014/06/07, begin */
    /* Modified for DTS2013090404247 by c00220250, 2013/09/04, begin */
    ...
    /* Modified for DTS2013090404247 by c00220250,2013/09/04, end */
	/* Modified for DTS2014060908426 by c00144034, 2014/06/07, end */
	ispv1_config_idi(pic_attr->in_fmt, is_bypass_isp, &reg3c13, &reg3c15);
    /* BEGIN: Modified for PN:DTS2013062405477 by w65960, 2013/7/17 */
	...
	/* Modified for DTS2014060908426 by c00144034, 2014/06/07, begin */
       ...
       /*BEGIN PN: DTS2014101000197, Added by l00269429, 2014-10-10 */
	if(isp_data.pro_mode && isp_data.pro_cap_expo){
			reg6 = reg6 & 0x0F;
	...
	/*END PN: DTS2014101000197, Added by l00269429, 2014-10-10 */
  ...
  /* BEGIN: Modified for PN:DTS2013062901533 by w65960, 2013/7/11 */
...
static int ispv1_start_capture_raw_with_preview_cmd(pic_attr_t *pic_attr, buffer_arr_t *buf_arr)
{
	...
	SETREG8(COMMAND_REG1, CAPTURE_STREAM_ENABLE
	                ...
	                | (buf_cnt-1));
	/* Modified  by w00199382 for DTS2013080903368, 2013/9/11, begin */
	...
	*/
	...
	return ret;
	/* Modified  by w00199382 for DTS2013080903368, 2013/9/11, end */
}
                ...
                u32             target_addr)
{
    ...
    print_info("enter %s.target_addr=0x%pK,zsl_proc_idx=%d",__func__,(void*)target_addr,this_ispdata->zsl_ctrl.zsl_proc_idx);
    ...
    /* modified by c00144034 for DTS20131212000873 begin */
    ...
    /* modified by c00144034 for DTS20131212000873 end */
#endif
...
//    spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
    ...
    if (0 != ret){
        ...
        /* modified by c00144034 for DTS2014062501645 */
...
/* Modified for DTS2013090404247 by c00220250, 2013/09/04, begin */
...
#else
	/* Modified  by w00199382 for DTS2013090208990, 2013/8/31, begin */
	if (cold_boot == true){
	...
	/* Modified  by w00199382 for DTS2013090208990, 2013/8/31, end */
#endif
/* Modified for DTS2013090404247 by c00220250, 2013/09/04 end */
/* Modified for DTS2014060908426 by c00144034, 2014/06/07, end */
...
static int ispv1_start_preview(pic_attr_t *pic_attr, camera_sensor *sensor, bool cold_boot, camera_scene scene)
{
    ...
    /* Enable INT before send cmd and disable INT at stop cmd, DTS2013082006383, modifier: z62576, end */
    /* Revised by c00220250 for DTS2014070202070, 2014-07-08, begin */
...
static int ispv1_start_proc_img(pic_attr_t *pic_attr, ipp_mode mode, buffer_arr_t *ipp_buf_arr)
{
    ...
    if (CAMERA_ZSL_OFFLINE == this_ispdata->zsl_ctrl.zsl_proc_mode){
        ...
        /* Modified  by w00199382 for DTS2013091806881, 2013/9/24, begin */
        spin_lock_irqsave(&isp_hw_data.data_queue_ref->queue_lock, lock_flags);
        ...
        if (!list_empty(source)) {
            ...
            /* modify queue empty judge method, DTS2013090909584, modifier: z62576, begin */
            spin_unlock_irqrestore(&isp_hw_data.data_queue_ref->queue_lock, lock_flags);
            /* modify queue empty judge method, DTS2013090909584, modifier: z62576, end */
            return -EINVAL;
        }
        ...
        print_debug("%s:framedst->phyaddr=0x%pK,dst_viraddr=0x%pK", __func__,(void*)framedst->phyaddr,(void*)dst_viraddr);
        ...
        print_debug("%s:framesrc->phyaddr=0x%pK,src_viraddr=0x%pK", __func__,(void*)framesrc->phyaddr,(void*)src_viraddr);
...
#endif
        spin_lock_irqsave(&isp_hw_data.data_queue_ref->queue_lock, lock_flags);
        /* Modified  by w00199382 for DTS2013091806881, 2013/9/24, end */
...
static int ispv1_start_process(pic_attr_t *pic_attr, u8 mode, buffer_arr_t *ipp_buf_arr)
{
	...
	if (mode == CAMERA_IPP_MODE_HDR) {
		...
		print_debug("****framesrc=0x%pK, idx=%d, flag=%d, phy=0x%pK****",
...
static int ispv1_start_capture(pic_attr_t *pic_attr, camera_sensor *sensor, int *ev, bool flash_on, camera_scene scene,	buffer_arr_t *buf_arr)
{
	...
	/* Modified  by w00199382 for DTS2013071902834, 2013/7/22, begin */
	isp_hw_data.frame_count = 0;
	/* Modified  by w00199382 for DTS2013071902834, 2013/7/22, end */
...
/* delete force overflow, DTS number:DTS2013070807499, modifier: z62576, begin */
/* for K3V2, there is some problem with frame control, so k3 use force overflow reg;
   after isp2.0, use new frame control, not need force overflow. */
    ...
    /* end */
#endif
/* delete force overflow, DTS number:DTS2013070807499, modifier: z62576, end */
...
static int ispv1_stop_capture(void)
{
	...
	/*revise for DTS2014082504156 by c00220250, 2014-09-02, begin */
	if(false == oneshot_capture)
	{
		ispv1_wait_for_mac_done();
	}
	/*revise for DTS2014082504156 by c00220250, 2014-09-02, end */
	ispv1_rsv_buf_init();
	oneshot_capture = true;
	...
	u32 size, i2c_length length, u8 mask, i2c_length addr_len)
{
	...
	do {
		...
		print_debug("write_seq_thr_isp buf[%pK], i2c_writen=%d, i2c_write_num=%d",
			(void*)buf, i2c_writen, i2c_write_num);
...
static void ispv1_set_i2c(struct i2c_t *i2c)
{
	...
	if (i2c->index == I2C_PRIMARY) {
        ...
        /* modify by c00144034 for DTS2013092510079 begin */
        ...
        phy_reg_writel(SOC_SC_OFF_BASE_ADDR,
                   ...
                   0x2);
        /* modify by c00144034 for DTS2013092510079 end */
    #endif
 ...
 **************************************************************************
 */
/* BEGIN PN:DTS2014052805616, Modified by l00183577, 2014-05-28*/
...
/* END PN:DTS2014052805616, Modified by l00183577, 2014-05-28*/
/*
 **************************************************************************
...
static int ispv1_frame_done_handler(camera_state state,u8 buf_sel)
{
	...
	if ((isp_hw_data.frame_count == 5) && (state == STATE_PREVIEW)) {
		/* modified by c00168914 for DTS2012041403018 20120620 begin */
		...
		/* modified by c00168914 for DTS2012041403018 20120620 end*/
	}
	...
	if (!list_empty(work_q)) {
		...
		/*added by y00215412 for DTS2013012304419 : set skip frames' ts zero */
		...
		/* added for DTS2013030101733 by j00212990 in 2013-03-04 */
		//frame->viraddr = NULL;
...
void flash_handler(void)
{
	...
	if(flash_flag == 1){
                /* DTS2014083001601 add by l00269429 for flash interface 20140901  begin */
                this_ispdata->flash_flow = FLASH_CAPFLASH_START;
                /* DTS2014083001601 add by l00269429 for flash interface 20140901  end */
...
static irqreturn_t ispv1_isr(int irq, void *dev_id)
{
    ...
    /* modified by c00144034 for DTS2013101002092 begin */
    ...
    /* modified by c00144034 for DTS2013101002092 end */
...
static int ispv1_hw_set_default (void)
{
	...
	/* added for DTS2012061807407 by j00212990 2012-06-16 using for display attribute of main camera */
	...
	/* added by y00215412 DTS2012122102699 2012-12-21. */
	isp_hw_data.flash_resume = false;
...
static int ispv1_switch_sensor_cmd(camera_sensor *sensor, stream_state state)
{
	...
	*/
	...
	print_info("stream_setting_size = %d , stream_setting = 0x%pK",stream_setting_size, (void*)stream_setting);
