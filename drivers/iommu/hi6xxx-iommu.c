...
#include <linux/hisi/hisi-iommu.h>//added for solving vpu_noc by y00182216 2015/8/5
#include <linux/io.h>
#include <linux/of.h>
#include <linux/clk.h>
#include <linux/ion.h>
...
//added for solving vpu_noc by y00182216 2015/8/5 start
...
//added for solving vpu_noc by y00182216 2015/8/5 end
...
static void smmu_enable(struct device *dev,struct hi6xxx_smmu_dev *smmu_dev)
{
	...
	if (smmu_ctrl){
		...
		else if (smmu_ddr_size == (3*SZ_1G) ){
			pr_info("SZ_3G the size of ddr\n");
			writel(be32_to_cpu(*smmu_ctrl)|(0x3<<2),(smmu_chip->reg_base + SMMU_CTRL_OFFSET));
		}
		else {
