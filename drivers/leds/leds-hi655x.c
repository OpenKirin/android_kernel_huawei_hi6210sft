...
static struct hisi_led_platform_data hi6552_leds = {
    ...
    .leds = {
        [0] = {
            .name = "red",
            .brightness = LED_OFF,
            .each_maxdr_iset = DR_BRIGHTNESS_FULL,
            .delay_on = 0,
            .delay_off = 0,
...
static int hisi_led_set(struct hisi_led_data *led, u8 brightness)
{
    ...
    case HISI_LED2:
        if (brightness == LED_OFF) {
            ...
            max_iset = hi6552_leds.leds[id].each_maxdr_iset;
            hisi_led_set_select_led(max_iset, id);
...
static int hisi_led_probe(struct platform_device *pdev)
{
    ...
    for (index = 0; index < HISI_LEDS_MAX; ++index) {
        ...
        if (!root) {
            dev_info(&pdev->dev, "there is no need to config hisilicon,hisi-led%d !\n", index);
            //goto err;
        }
        ret = of_property_read_u32(root, "hisilicon,each_max_iset", &hi6552_leds.leds[index].each_maxdr_iset);
        if (ret < 0) {
            dev_info(&pdev->dev, "config led%d's each_maxdr_iset failure! use default value.\n", index);
            hi6552_leds.leds[index].each_maxdr_iset = DR_BRIGHTNESS_FULL;
            //goto err;
        }
        dev_info(&pdev->dev, "config led%d's each_maxdr_iset=%d.\n", index,hi6552_leds.leds[index].each_maxdr_iset);
    }
