		...
		do {
			...
			if (failed & (1u << i)) {
				...
				case BASE_JD_EVENT_STOPPED:
					...
					dev_warn(kbdev->dev, "Job done with completion code [BASE_JD_EVENT_STOPPED]");
					break;
				case BASE_JD_EVENT_NOT_STARTED:
					...
					/* fall throught */
					dev_warn(kbdev->dev, "Job done with completion code [BASE_JD_EVENT_TERMINATED]");
					break;
				default:
					dev_warn(kbdev->dev, "error detected from slot %d, job status 0x%08x (%s)", i, completion_code, kbase_exception_name(completion_code));
					printk("MALI-MIDGARD: [JOB]current frequence is :%lld \n", kbdev->devfreq->previous_freq);
					/* Report as panic if device run on factory mode */
