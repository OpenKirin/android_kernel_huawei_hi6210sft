...
#define KBASE_HI3630_PLATFORM_GPU_REGULATOR_NAME      "g3d"
#define KBASE_HI3630_GPU_IRDROP_ISSUE	1
#if KBASE_HI3630_GPU_IRDROP_ISSUE
#define KBASE_HI3630_GPU_TURBO_FREQ		360000000
#endif /* KBASE_HI3630_GPU_IRDROP_ISSUE */
...
static void handle_switch_policy(struct work_struct *work)
{
	...
	if (!strncmp(policy->name, "always_on", strlen("always_on"))) {
		...
		printk("[mali-midgard] dvfs call clk_set_rate in switch policy, old_freq = %lu -----> new_freq = %lu \n", kbdev->devfreq->previous_freq, freq);
			      ...
			      u32 flags)
{
	...
	if(old_freq <= KBASE_HI3630_GPU_TURBO_FREQ && freq > KBASE_HI3630_GPU_TURBO_FREQ ) {
		sw_policy.freq = freq;
	...
	if (cur_policy == NULL || ((freq > KBASE_HI3630_GPU_TURBO_FREQ)
...
static mali_bool kbase_platform_init(struct kbase_device *kbdev)
{
	...
	kbdev->regulator = devm_regulator_get(dev, KBASE_HI3630_PLATFORM_GPU_REGULATOR_NAME);
...
static struct kbase_platform_config hi3630_platform_config = {
