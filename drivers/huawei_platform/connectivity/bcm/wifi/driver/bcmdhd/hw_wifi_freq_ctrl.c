...
static int get_cpu_freq_raw(int freq, uint* value)
{
    ...
    if (IS_ERR(filp)) {
        printk("get_cpu_freq_raw:¡¡freq¡¡= %d error\n", freq);
        return -1;
    }
    old_fs = get_fs();
    set_fs(KERNEL_DS);
...
static int set_cpu_freq_raw(int freq, uint value)
{
    ...
    if (IS_ERR(filp)) {
        printk("set_cpu_freq_raw:¡¡freq¡¡= %d error\n", freq);
        return -1;
    }
    old_fs = get_fs();
    set_fs(KERNEL_DS);
...
static int get_ddr_freq_raw(int freq, uint* value)
{
    ...
    if (IS_ERR(filp)) {
        printk("get_ddr_freq_raw:¡¡freq¡¡= %d error£\\n", freq);
        return -1;
    }
    old_fs = get_fs();
    set_fs(KERNEL_DS);
...
static int set_ddr_freq_raw(int freq, uint value)
{
    ...
    if (IS_ERR(filp)) {
        printk("set_ddr_freq_raw:¡¡freq¡¡= %d error£Ün", freq);
        return -1;
    }
    old_fs = get_fs();
    set_fs(KERNEL_DS);
...
static int set_ddr_freq(uint value, int type)
{
    ...
    if(type == FREQ_UPGRADE) {
        ...
        printk("error set_ddr_freq, invalid types\n");
        return -1;
    }
...
static void hw_wifi_release_freq_lock_work(struct work_struct *work)
{
    ...
    if (NULL == fl_control_ptr) {
        printk("hw_wifi_release_freq_lock_work  NULL point error!\n");
        return;
    }
...
static void hw_wifi_release_freq_lock_timer(unsigned long data)
{
    ...
    if(NULL == fl_control_ptr) {
        printk("hw_wifi_release_freq_lock_timer  NULL point error!\n");
        return;
    }
...
static void  hw_wifi_do_freq_lock_work(struct work_struct *work){
    ...
    if (NULL == fl_control_ptr) {
        printk("hw_wifi_do_freq_lock_work  NULL point error!\n");
        return;
    }
    ...
    if ((g_speed_freq_level == NULL) ||
        ...
        (0 == req_lock_level)) {
        printk("hw_wifi_do_freq_lock_work  invalid lock_level error!\n");
        mutex_unlock(&fl_control_ptr->lock_freq_mtx);
        return;
    }
...
static int get_rate_freq_data_from_dts()
{
    ...
    if (np == NULL) {
		printk("Unable to find bcm_wifi power\n");
		return -1;
	}

    prop = of_find_property(np, propname, NULL);
    if (prop == NULL) {
        printk("Can not get transfer rate data\n");
        return -1;
    }
    ...
    if(!p_table) {
        printk("Unable to alloc enough buffer\n");
        return -1;
    }
...
int  hw_wifi_freq_ctrl_init(void)
{
    ...
    if (!freq_lock_control_ptr) {
        printk("hw_wifi_wifi_freq_ctl_init kzalloc error!\n");
        goto out;
    }

    ret = get_rate_freq_data_from_dts();
    if(ret != 0) {
        printk("Can not get transfer rate and CPU/DDR freq table\n");
        goto cleanup;
    }
    ...
    if (!freq_lock_control_ptr->freq_ctl_wq) {
        printk("hw_wifi_wifi_freq_ctl_init  create_singlethread_workqueue  error!\n");
...
int  hw_wifi_freq_ctrl_destroy(void)
{
    ...
    if (NULL == freq_lock_control_ptr) {
        printk("freq_lock_control_ptr NULL pointer!\n");
        return 0;
    }
...
static int hw_wifi_speed_calc_process (unsigned long bytes) {
    ...
    if ((g_speed_freq_level == NULL) ||
        (g_freq_lock_num == 0) ) {
        printk("Unable to get transfer data, may be is not config is dts\n");
        return 0;
    }
    ...
    if (level_idx > 0) {
        if (NULL == freq_lock_control_ptr) {
            printk("hw_wifi_speed_calc_process  NULL point error!\n");
            return -1;
        }
