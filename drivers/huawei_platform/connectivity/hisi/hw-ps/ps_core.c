...
/* DTS2013112001862   Hi110x bug fix liuwenlong/00244150 2013/11/20 begin */
...
/* DTS2013112001862   Hi110x bug fix liuwenlong/00244150 2013/11/20 end */
...
int32 mod_beat_timer(uint8 on)
{
    ...
    if (on)
    {
        ...
        /*DTSDTS2013103108671 modify for bfg beat timer begin*/
        if (timer_pending(&ps_core_d->beat_timer))
        {
            atomic_set(&ps_core_d->bfg_beat_flag, 0);
        }
        mod_timer(&ps_core_d->beat_timer, jiffies + BEAT_TIME_ACTIVE * HZ);
        /*DTSDTS2013103108671 modify for bfg beat timer end*/
    }
    else
    {
        /*del timer*/
    ...
    if (0 == atomic_read(&ps_core_d->bfg_beat_flag))
    {
        ...
        if (true == ps_core_d->allow_reset_device)
        {
            if (0 == ps_core_d->in_reset_device_process)
            {
				...
				/* DTS2013121901713   Hi110x bug fix liuwenlong/00244150 2013/12/19 begin */
                if ((0 == ps_core_d->ps_pm->bfg_gpio_state_get()) && \
				...
				/* DTS2013121901713   Hi110x bug fix liuwenlong/00244150 2013/12/19 end */
            }
        }
    }
...
int32 ps_decode_packet_func(struct ps_core_s *ps_core_d, uint8 *buf_ptr)
{
    ...
    case PACKET_RX_FUNC_SYS:
		...
		/* DTS2014042602454 Hi110x bug fix liuwenlong/00244150 2014/05/16 begin */
		...
		/* DTS2014042602454 Hi110x bug fix liuwenlong/00244150 2014/05/16 end */
...
int32 ps_check_packet_head(struct ps_core_s *ps_core_d, uint8 *buf_ptr, int32 count)
{
    ...
    {   /* if count less then sys packet lenth and continue */
        ...
        if (PACKET_RX_FUNC_BFG_DBG >= ps_core_d->rx_pkt_type)
        {
            ...
            /* DTS2013112801045: Hi110x bug fix zwx144390 2013/11/28 begin */
    ...
    /* DTS2013112801045: Hi110x bug fix zwx144390 2013/11/28 end */
    return RX_PACKET_ERR;
}
...
/* DTS2013122503814   Hi110x bug fix zwx144390 2013/12/26 begin */
#ifdef DEBUG_USE
    vfs_write(ps_core_d->rx_data_fp, data, count, &ps_core_d->rx_data_fp->f_pos);
#endif
/* DTS2013122503814   Hi110x bug fix zwx144390 2013/12/26 end */
