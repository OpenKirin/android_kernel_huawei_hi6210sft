...
static void *create_and_get_mt_pdata(struct device_node *dev_node)
{
	...
	if (!rc) {
		...
		TS_LOG_INFO("%s %d:[cy,x_max] not found,will ignore, rc = %d\n",
	...
	if (!rc) {
		...
		TS_LOG_INFO("%s %d:[cy,y_max] not found,will ignore, rc = %d\n",
					__func__, __LINE__, rc);
	}
	...
	if (!rc) {
		...
		TS_LOG_INFO("%s %d:[cy,p_max] not found,will ignore, rc = %d\n",
					__func__, __LINE__, rc);
	}
	...
	if (!rc) {
		...
		TS_LOG_INFO("%s %d:[cy,slots_max] not found,will ignore, rc = %d\n",
					__func__, __LINE__, rc);
	}
        ...
        struct device_node *core_node)
{
	...
	rc = of_property_read_u32(core_node, "cy,input_register_early", &value);
	if (!rc){
		pdata->input_register_early = value;
		TS_LOG_INFO("%s:input_register_early = %d\n",__func__,pdata->input_register_early);
	} else {
		pdata->input_register_early = 0;
		TS_LOG_INFO("%s,[cy,input_register_early] read fail, rc = %d.\n",__func__, rc);
	}
