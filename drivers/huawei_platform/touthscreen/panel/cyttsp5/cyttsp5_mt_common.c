...
static int cyttsp5_mt_attention(struct device *dev)
{
	...
	if (!md->si) {
		...
		if (!md->si) {
			TS_LOG_INFO("%s:md->si request again fail\n",__func__);
			return -EINVAL;
		}
	}
...
static int cyttsp5_startup_attention(struct device *dev)
{
	...
	TS_LOG_INFO("%s:in \n",__func__);
	if (!md->si) {
		TS_LOG_INFO("%s:md->si is null, request again\n",__func__);
		md->si = _cyttsp5_request_sysinfo(dev);
		if (!md->si) {
...
static int cyttsp5_setup_input_device(struct device *dev)
{
	...
	if ( md->si ){
		...
		TS_LOG_INFO("%s:input_register_early config true,use p_max=%d slots_max=%d in DTS\n",
    ...
    if (md->pdata->flags & CY_MT_FLAG_VKEYS) {
		...
		if ( md->si ) {
			...
			TS_LOG_INFO("%s:input_register_early config true,use x_max=%d y_max=%d in DTS\n",
...
int cyttsp5_mt_probe(struct device *dev)
{
    ...
    if (md->si) {
		...
		if (cd->pdata->core_pdata->input_register_early) {
			TS_LOG_INFO("%s:input_register_early config true,will register input devices nowait FW upgrade\n",__func__);
			rc = cyttsp5_setup_input_device(dev);
			if (rc) {
