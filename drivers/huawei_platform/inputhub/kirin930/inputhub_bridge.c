...
int inputhub_mcu_connect( void )
{
    ...
    if (!(mbox = hisi_mbox_get(HISI_MAILBOX_RP_IOM3, nb))) {
        ...
        dmd_log_report(DSM_SHB_ERR_MAIN_INIT, __func__, "failed to get mailbox\n");
        kfree((void *)nb);
        return -ENODEV;
    }
    mbox->nb=nb;
...
static void __iomem *iom3_crgperi_base;
#define CRG_PERI_BASE_IOM3   0xFFF35000
#define CRGPERIPHIOM3CTRL_IOM3 0x1F0   /*IOM3 NMI,bit4*/
void noc_set_iom3_nmi()
{
	unsigned int value;

	hwlog_err("%s  enter\n", __func__);

	value = readl(iom3_crgperi_base + CRGPERIPHIOM3CTRL_IOM3);
	value |= (0x1 << 4);
	writel(value, (iom3_crgperi_base + CRGPERIPHIOM3CTRL_IOM3));
	value &= ~(0x1 << 4);
	writel(value, (iom3_crgperi_base + CRGPERIPHIOM3CTRL_IOM3));

	if(!dsm_client_ocuppy(shb_dclient)){
		dsm_client_record(shb_dclient, "%s, NOC erro recovery", __func__);
		dsm_client_notify(shb_dclient, DSM_SHB_ERR_IOM3_UNKNOW);
	}
	hwlog_err("%s  leave\n", __func__);
}
...
void boot_iom3(void)
{
    ...
    if(!tx_task) {
        ...
        dmd_log_report(DSM_SHB_ERR_MAIN_INIT, __func__, "failed to alloc task\n");
        return;
    }
    if (hisi_mbox_msg_send_async(lpm3_mbox,tx_task) != 0) {
        hwlog_err("hisi_mbox_msg_send_async error in %s\n", __func__);
        dmd_log_report(DSM_SHB_ERR_MAIN_INIT, __func__, "hisi_mbox_msg_send_async error\n");
    }
}
...
static int  inputhub_mcu_init(void)
{
	...
	iom3_crgperi_base = ioremap(CRG_PERI_BASE_IOM3, SZ_4K);
	if (!iom3_crgperi_base) {
