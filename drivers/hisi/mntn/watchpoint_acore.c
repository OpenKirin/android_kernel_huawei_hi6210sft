...
static int watchpoint_check_caller(void *addr, void *regs, void *caller)
{
    ...
    mlog_i("addr=0x%pK, regs=0x%pK, caller=0x%pK", addr, regs, caller);
...
{
    mlog_w("tst cb called, addr=0x%x, regs=0x%pK, usrp=0x%pK", addr, regs, user_param);
...
static int wp_std_cb_func(unsigned int addr, void* regs, void* user_param)
{
    ...
    mlog_i("std cb called, addr=0x%x, regs=0x%pK, usrp=0x%pK", addr, regs, user_param);
    ...
    if (is_in_stack) {
        ...
        /* to die */
        mlog_n("Unexpected caller, addr=0x%x, regs=0x%pK, usrp=0x%pK", addr, regs, user_param);
        ret = 1;
    }
...
static int wp_tgetmem_write(struct file *file, const char __user *user_buf, size_t count, loff_t *ppos)
{
    ...
    if ('m' == kn_buf[0]) {
        ...
        /* free */
        num = sscanf(kn_buf, "f 0x%pK %u", &addr, &type);
        if (2 == num) {
