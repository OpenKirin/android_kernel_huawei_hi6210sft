...
void create_dump_phy_mem_proc_file(char *name, unsigned int phy_addr, size_t size)
{
    ...
    balong_create_memory_proc_entry(name, S_IRUSR | S_IRGRP, &dump_phy_mem_proc_fops, info);
...
\
    ...
    info->size = iosize;\
\
    balong_create_ ## directory ## _proc_entry(string, S_IRUSR | S_IRGRP, &dump_phy_mem_proc_fops, info);\
\
    return 0;\
}\
\
...
static int __init sram_mcu_mem_dump_proc_init(void)
{
    ...
    balong_create_memory_proc_entry("sram_mcu", S_IRUSR | S_IRGRP, &sram_mcu_mem_proc_fops, NULL);
...
/* BEGIN: Added by y65256 for DTS2013090605137, 2013/9/9 */
...
{
    balong_create_reboot_proc_entry("reboot", S_IRUSR | S_IRGRP, &reboot_proc_fops, NULL);
    return 0;
}
...
{
    balong_create_stats_proc_entry("close", S_IRUSR | S_IRGRP, &proc_balong_reset_fops, NULL);
