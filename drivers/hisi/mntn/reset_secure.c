...
int check_vrl_valid(unsigned int buf)
{
    ...
    if (NULL != vrl_header && vrl_header->magicNumber == DX_VRL_MAGIC_NUMBER_DEFAULT_VALUE) {
        return OK;
    }
    else {
        return ERROR;
               ...
               unsigned int *image_size)
{
    ...
    if (NULL == vrl_data || NULL == vrl_size || NULL == image_num)
    {
        return ERROR;
    }
