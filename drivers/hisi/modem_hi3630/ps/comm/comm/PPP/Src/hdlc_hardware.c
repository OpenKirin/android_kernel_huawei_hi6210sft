...
#include "TTFComm.h"
#if (SC_CTRL_MOD_6210_SFT  == SC_CTRL_MOD)
...
VOS_UINT32 PPP_HDLC_HARD_Init(VOS_VOID)
{
    ...
    HDLC_IP_BASE_ADDR       = PPP_IO_ADDRESS(ulBaseAddr);

    if (VOS_NULL_PTR == HDLC_IP_BASE_ADDR)
    {

        PPP_MNTN_LOG1(PS_PID_APP_PPP, 0, PS_PRINT_ERROR,
                      "PPP_HDLC_HARD_Init HDLC base addr is null,0x%x\r\n",
                      ulBaseAddr);
    ...
    g_ulHdlcScCtrlBaseAddr  = PPP_IO_ADDRESS(ulBaseAddr);
#endif

    if (VOS_NULL_PTR == g_ulHdlcScCtrlBaseAddr)
    {
        PPP_MNTN_LOG1(PS_PID_APP_PPP, 0, PS_PRINT_ERROR,
                      "PPP_HDLC_HARD_Init HDLC SCCTRL base addr is null,0x%x\r\n",
                      ulBaseAddr);
        return VOS_ERR;
    }
