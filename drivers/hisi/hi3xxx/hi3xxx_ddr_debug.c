	...
	char *buf)
{
	...
	size += snprintf(buf + size, PAGE_SIZE - size, "pgd_tmp = 0x%pK\n", pgd_tmp);
	...
	size += snprintf(buf + size, PAGE_SIZE - size, "pud_tmp = 0x%pK\n", pud_tmp);
	...
	size += snprintf(buf + size, PAGE_SIZE - size, "pmd_tmp = 0x%pK\n", pmd_tmp);
	if(pmd_none(*pmd_tmp)){
	...
	size += snprintf(buf + size, PAGE_SIZE - size, "pte_tmp = 0x%pK\n", pte_tmp);
	size += snprintf(buf + size, PAGE_SIZE - size, "*pte_tmp = 0x%pK\n", (void *)*pte_tmp);
